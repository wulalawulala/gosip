	...
	Describe("construct", func() {
		Context("from net.UDPConn", func() {
			It("should set connection params", func() {
				...
				conn := transport.NewConnection(sUdpConn, "dummy", transport.WithLogger(logger))
		...
		Context("from net.TCPConn", func() {
			It("should set connection params", func() {
				...
				conn := transport.NewConnection(sTcpConn, "dummy", transport.WithLogger(logger))
	...
	Describe("read and write", func() {
		...
		Context("with net.UDPConn", func() {
			It("should read and write data", func() {
				...
				sConn := transport.NewConnection(sUdpConn, "dummy", transport.WithLogger(logger))
				cConn := transport.NewConnection(cUdpConn, "dummy", transport.WithLogger(logger))
